...
- 等待球不模糊：等待球（`lyrics_instrumental_root`）始终清晰显示，不受模糊效果影响
- 等待球阶段渐进模糊：歌曲开头等待动画出现时，歌词行按位置渐进施加模糊（blur=12, 16, 20...）
- 双行高亮支持（同时高亮多行时均不模糊）
...
模块通过视图 ID 区分不同类型的非歌词行，实现解耦控制：

- **等待球**（`lyrics_instrumental_root`）：始终不模糊，保持清晰显示
- **创作者行**（无 ID 的最后一个子 View）：跟随最后一行歌词的模糊深度

两者的模糊逻辑完全独立：
- 等待球的出现与否不影响创作者行的模糊计算
- 创作者行始终跟随歌词行的模糊状态，与等待球无关
