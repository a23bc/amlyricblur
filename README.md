...
此外还 Hook 了 
otifyWordHighlight(IIIZ)V 以支持逐字歌词的高亮累加。

### 双行高亮过渡

高亮行切换时，使用 ffectiveIds = currentHighlight + previousHighlight 作为有效高亮集合。这确保了双行高亮从 [10,11] 切换到 [11] 时，第10行不会立即被重新模糊，避免过渡闪烁。
...
APK 输出在 pp/build/outputs/apk/debug/app-debug.apk
...
- org.luckypray:dexkit:2.0.3 — [DexKit](https://github.com/LuckyPray/DexKit) 库（预留，运行时通过 dalvik.system.DexFile 进行 APK 扫描）
